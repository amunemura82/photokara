// photokara diagnose.js v4 — tool_use固定・9軸強制・列ズレ防止

const GAS_URL = 'https://script.google.com/macros/s/AKfycbweNivDBiiImK4qk4aIsiQIGbs_9zWuRADGhe5Bc8jYubAc5hTebuIBr2osFmyzFopC/exec';
...
async function uploadToCloudinary(base64Data, index = 0) {
  ...
  // 署名はアルファベット順（folder, public_id, timestamp）
  ...
  catch { throw new Error(`Cloudinary response not JSON: ${text}`); }

  if (!data?.secure_url) throw new Error(`Cloudinary missing secure_url: ${text}`);
  return data.secure_url;
}

// 9軸の正規名称（この順序でGASに送る）
const AXIS_NAMES = [
  '明るさ', 'コントラスト', 'シャープさ', '価値表現力',
  '質感・素材感', '清潔感', '誠実度', 'テキスト訴求力', '解像度・画質'
];
...
function normalizeDiagnosis(input) {
  ...
  // 入力軸を名前でマップ化
  const axMap = {};
  (Array.isArray(input?.axes) ? input.axes : []).forEach(ax => {
    if (ax?.name) axMap[ax.name] = ax;
  });

  // 9軸すべてを正規名称で出力（欠損は0埋め）
  out.axes = AXIS_NAMES.map(name => {
    const ax = axMap[name] || {};
    return {
      name,
      score:   Math.max(0, Math.min(100, typeof ax.score === 'number' ? ax.score : Number(ax.score ?? 0))),
...
export default async function handler(req, res) {
  ...
  // tool_use で JSON.parse不要・9軸を強制
  const tools = [{
    name: 'emit_diagnosis',
    description: 'Return diagnosis result. axes must contain ALL 9 items: 明るさ, コントラスト, シャープさ, 価値表現力, 質感・素材感, 清潔感, 誠実度, テキスト訴求力, 解像度・画質.',
    input_schema: {
      type: 'object',
  ...
  if (validImages.length > 0) {
    try {
      ...
      console.error('[Cloudinary]', cloudinaryError);
    }
  }
